...
const PracticeFAQ = ({ faqs, schemaEnabled = true }: PracticeFAQProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-[-15%] left-[-10%] w-[500px] h-[500px] bg-[#C5A869]/8 rounded-full blur-[100px] pointer-events-none" />
          ...
          {faqs.map((faq, index) => (
            <div
              ...
              className={`faq-item py-8 ${
                index < faqs.length - 1
                  ? 'border-b border-[#C5A869]/15'
                  : ''
              }`}
            >
              <h3 className="font-serif font-semibold text-gq-dark text-xl sm:text-2xl mb-3 leading-snug">
                {faq.question}
              </h3>
              <p className="font-sans text-gq-dark/70 text-base sm:text-lg leading-relaxed">
                {faq.answer}
              </p>
              {faq.statute && (
                <p className="font-sans text-[#C5A869]/80 text-sm italic mt-3">
                  {faq.statute}
                </p>
              )}
            </div>
